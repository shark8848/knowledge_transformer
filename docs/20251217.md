...
- 视频：字幕/ASR/VLLM + 关键帧描述 → transcript → chunk（章节/时间段）→ embedding
...
- 规模：$N$ 为 chunk 总数，$p=0.7$ 为 public 占比。
- 文本：平均长度 $L\approx600$ tokens，去重后词项数 $t\approx250\text{--}350$。
- 向量：维度 $d=768$，HNSW 参数 $M=16$、$ef_{\text{construct}}=200$、$ef_{\text{search}}=80$。
- 查询：BM25 `size=200`，kNN `k=150`，`num_candidates=2000`，RRF `topM=200`，rerank `topR=100`，返回 `topK=20`（见 [docs/20251217.md](docs/20251217.md#L269-L310)）。

### 22.2 构建/写入复杂度

- Chunk + embedding 生成：$O(N \cdot L) + O(N \cdot d)$。
- 倒排（BM25）建索：$O(N \cdot t)$，$t$ 与 $L$ 近线性。
- HNSW 建图：$O(N \cdot M \cdot \log N)$，严格上界 $O(N \cdot M \cdot ef_{\text{construct}})$。

### 22.3 查询复杂度（单次请求，四路 Hybrid）

- BM25：$O\!\left(\sum_{q_i\in q}\text{df}(q_i)\right)$，在 WAND/BMW 剪枝下近似 $O(k\log N + V)$，$V$ 为被访问文档数。
- HNSW kNN：$O\big(ef_{\text{search}} \cdot (M + \log N)\big)$；以 $ef_{\text{search}}=80$、$M=16$ 估计每索引需约 $80\times(16+\log_2N)$ 次相似度评估。
- 融合：四路结果去重 + RRF，$O(k \log k)$。
- Rerank：$O(R \cdot L)$，其中 $R=topR$；若开启 LLM rerank，$O(R_{\text{llm}} \cdot L_{\text{llm}})$（通常 $R_{\text{llm}}\ll R$）。

### 22.4 空间复杂度（不含副本）

- 向量：$O(N \cdot d)$；float32 约 $3\,\text{KB}$/向量。
- HNSW 图：边数 $\approx N\cdot M$，以 4B/边计，$N=10^6$ 时约 $640\,\text{MB}$，$N=10^7$ 时约 $6.4\,\text{GB}$，再乘 1.1~1.3 元数据系数。
- 倒排：$O(N \cdot t)$；$t\approx300$，posting 若 12B，$N=10^6$ 约 3.6GB，$N=10^7$ 约 36GB（压缩后可减半）。
- _source/metadata：若每 chunk ~1.2\text{--}1.5KB，$N=10^6$ 约 1.2\text{--}1.5GB，$N=10^7$ 约 12\text{--}15GB。

### 22.5 查询计算示例（默认参数）

**场景 A（MVP，单索引，$N=10^6$）**

- HNSW 相似度评估：$80\times(16+\log_2 10^6)\approx80\times36\approx2.9k$ 次，每次 768 维点积，算子量 \~2.2M multiply-add。
- BM25：假设 5 个 query term，平均 $\text{df}=5\times10^3$，剪枝后实际访问 $V\approx4\times10^4$ postings，可在 200 返回窗口内完成。
- 融合+rerank：融合 200 条，rerank 100 条，cross-encoder 复杂度 $O(100 \cdot 600)$ token 级；LLM rerank 关闭（默认）。
- 延迟拆分：BM25 \~50-120ms，kNN \~30-80ms，融合+rerank \~150-400ms，总体 230-600ms（需压测校准）。

**场景 B（Scale，public/private 拆分，$N=10^7$, public:private=7:3）**

- public 路径（$N\approx7\times10^6$）：$80\times(16+\log_2 7\times10^6)\approx80\times39\approx3.1k$ 次相似度评估；无 ACL filter。
- private 路径（$N\approx3\times10^6$）：$80\times(16+\log_2 3\times10^6)\approx80\times38\approx3.0k$ 次相似度评估；附加 terms scope filter（用户 scopes 数目有限，接近 $O(k)$）。
- BM25 访问量随索引划分线性缩减；public 与 private 并行，fanout 受 shard 数影响（见 [docs/20251217.md](docs/20251217.md#L228-L258)）。
- 总融合规模仍是 200→100→20，故 rerank 成本不变；整体延迟主要由更大向量索引与更高 shard fanout 决定，需按分片/副本规划及 cache 命中率压测。

### 22.6 调优提示

- 降时延：减小 `num_candidates`/`k`/`topR`，或调高缓存（页缓存、向量内存命中率）。
- 降存储：向量量化（PQ/IVF-PQ/FP16）、_source 压缩、冷热分层；倒排启用最佳压缩与强制合并。
- 权限友好：优先 public/private 拆分，必要时按业务/租户再拆以降低 terms scope 集合与 shard fanout。