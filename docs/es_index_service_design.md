...
## 2. 代码结构
- FastAPI 应用与路由：`src/es_index_service/app.py`，`src/es_index_service/api/routes.py`
- 配置与环境：`src/es_index_service/config.py`（Pydantic Settings，前缀 `ES_INDEX_SERVICE_`，兼容旧前缀）
- 任务与客户端：`src/es_index_service/tasks.py`（Celery 任务），`src/es_index_service/clients.py`（ES HTTP 客户端，bulk 含 `Content-Type: application/x-ndjson`）
- gRPC 桥：`src/es_index_service/grpc_server.py`（通用 JSON handler）
- 映射模板：`config/kb_chunks_v1_mapping.json`
- 管理脚本：`start_es_index_service.sh`、`stop_es_index_service.sh`、`show_es_index_service.sh`

## 3. 配置与环境变量（核心）
前缀 `ES_INDEX_SERVICE_`（向后兼容 `ES_SERVICE_`），嵌套用双下划线 `__`。
...
示例（自签证书环境）：
```
ES_INDEX_SERVICE_ES__ENDPOINT=https://localhost:9200
ES_INDEX_SERVICE_ES__USERNAME=elastic
ES_INDEX_SERVICE_ES__PASSWORD=<password>
ES_INDEX_SERVICE_ES__VERIFY_SSL=false
ES_INDEX_SERVICE_API_PORT=8085
ES_INDEX_SERVICE_GRPC_PORT=9105
```
...
## 7. 直接 Python 调用示例
```python
from es_index_service.config import get_settings
from es_index_service.clients import ESClient
s = get_settings()
es = ESClient(s)
...
**快速写入现有索引/别名**
```python
from es_index_service.tasks import celery_app
...
**可靠性与性能提示**
- 控制并发/预取：`ES_INDEX_SERVICE_CELERY__PREFETCH_MULTIPLIER`（默认 1）避免过多预取导致内存占用。
- 任务超时：`ES_INDEX_SERVICE_CELERY__TASK_TIME_LIMIT_SEC`（默认 300）。
- 队列隔离：如需区分写入/重建，可改 `ES_INDEX_SERVICE_CELERY__DEFAULT_QUEUE` 或在 send_task 指定 `queue`。
- Worker 命名：`ES_INDEX_SERVICE_WORKER_NAME` 便于可观测。
- 错误重试：可在上层调用方捕获 `res.state` 及 `res.result`，根据业务补偿；当前任务未内置重试策略。

## 9. 运行与脚本
- 启动：`start_es_index_service.sh`（API + Celery + gRPC，日志 `logs/`，pid `.run/`）
- 停止：`stop_es_index_service.sh`
- 状态：`show_es_index_service.sh`（进程、/healthz、ES 健康、Celery ping）

## 10. 依赖与注意事项
- Elasticsearch 8.19.7 需安装插件：`analysis-ik`、`analysis-pinyin`，否则创建索引会报 unknown filter。
- 自签证书环境可设 `ES_INDEX_SERVICE_ES__VERIFY_SSL=false`（或使用兼容前缀 `ES_SERVICE_`），导入 CA 后亦可开启校验。
...
## 11. 故障与排查
- 401：检查 ES 用户密码或 `ES_INDEX_SERVICE_ES__USERNAME/PASSWORD` 是否正确（兼容旧前缀）。
