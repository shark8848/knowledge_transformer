...
SCRIPT_TAG="es-index-service-show"
resolve_bin() {
  local name="$1"
...
echo "== ES Index Service Process Status =="
printf "%-22s %s\n" "ES Index Service API" "$(is_running "$RUN_DIR/es-index-service-api.pid")"
printf "%-22s %s\n" "ES Index Service Celery" "$(is_running "$RUN_DIR/es-index-service-celery.pid")"
printf "%-22s %s\n" "ES Index Service gRPC" "$(is_running "$RUN_DIR/es-index-service-grpc.pid")"
